...
APPLICATION_NAME = "Franchises names"
...
def editFranchise(franchise_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editFranchise.html', franchise=editedFranchise)
...
def deleteFranchise(franchise_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(
            url_for('showFranchises', franchise_id=franchise_id))
    else:
        return render_template(
            'deleteFranchise.html', franchise=franchiseToDelete)
...
@app.route(
    '/franchise/<int:franchise_id>/team/new/', methods=['GET', 'POST'])
def newTeamPlayer(franchise_id):
    if 'username' not in login_session:
	return redirect('/login')
    if request.method == 'POST':
        newPlayer = TeamPlayer(name=request.form['name'], description=request.form[
                           'description'], price=request.form['price'], course=request.form['course'], franchise_id=franchise_id)
...
def editTeamPlayer(franchise_id, team_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editteamplayer.html', franchise_id=franchise_id, team_id=team_id, player=editedPlayer)
...
def deleteTeamPlayer(franchise_id, team_id):
    if 'username' not in login_session:
	return redirect('/login')
    playerToDelete = session.query(TeamPlayer).filter_by(id=team_id).one()
    if request.method == 'POST':
        session.delete(playerToDelete)
        session.commit()
        return redirect(url_for('showTeam', franchise_id=franchise_id))
    else:
        return render_template('deleteteamplayer.html', player=playerToDelete)
